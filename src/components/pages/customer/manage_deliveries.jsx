      ...
      <div className="bg-gray-900 rounded-lg shadow-xl p-6 z-50 w-96 max-h-[90vh] overflow-y-auto border border-gray-800">
        ...
        <form onSubmit={handleAddUpdateDelivery}>
          <div className="space-y-4">
            {/* Display current delivery information (read-only) */}
            <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
              <h3 className="text-blue-400 font-semibold mb-2">Current Delivery Details</h3>
              <div className="space-y-2 text-sm text-gray-300">
                <p><span className="font-medium">Order Origin:</span> {currentDelivery?.order_details?.origin || "N/A"}</p>
                <p><span className="font-medium">Driver:</span> {currentDelivery?.driver_details?.user_name || "N/A"}</p>
                <p><span className="font-medium">Vehicle:</span> {currentDelivery?.vehicle_details?.plate_number || "N/A"}</p>
                <p><span className="font-medium">Warehouse:</span> {currentDelivery?.warehouse?.warehouse_detail?.location || "N/A"}</p>
              </div>
            </div>
...
const handleAddUpdateDelivery = async (e) => {
  ...
  try {
    if (currentDelivery) {
      // For existing deliveries, update status while preserving other values
      const updatedData = {
        status: e.target.status.value,
        // Include current delivery values to ensure they're preserved - extract IDs only
        order: currentDelivery.order || currentDelivery.order_details?.id,
        driver: currentDelivery.driver?.id || currentDelivery.driver_details?.id,
        vehicle: currentDelivery.vehicle || currentDelivery.vehicle_details?.id,
      };
      
      console.log("Updating delivery with data:", updatedData);
      console.log("Current delivery object:", currentDelivery);
